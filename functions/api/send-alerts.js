...
function isDirOk(dir, goodDirs, badDirs) {
  ...
  if (!goodDirs?.length) return true;
...
async function getForecast(spot, supabaseUrl, serviceKey, forecastUrl, ensUrl, openmeteoKey) {
  try {
  ...
  const ensFullUrl = `${ensUrl}?hourly=wind_speed_10m&models=gfs05&forecast_days=${ENS_DAYS}${loc}${openmeteoKey}`;

  const [detArr, ensArr] = await Promise.all([
    fetch(detUrl).then(r => r.json()).then(d => Array.isArray(d) ? d : [d]),
    fetch(ensFullUrl).then(r => r.json()).then(d => Array.isArray(d) ? d : [d]),
  ]);
...
function checkAlert(forecast, alert, goodDirs, badDirs) {
  ...
  for (const [date, hours] of Object.entries(byDay)) {
    ...
    if (good.reduce((s, h) => s + h.precip, 0) > 2) continue;
...
export async function onRequest(context) {
  ...
  const secret = request.headers.get("x-alert-secret") ?? new URL(request.url).searchParams.get("secret");
  if (!secret || secret !== env.ALERT_SECRET) {
    return new Response(JSON.stringify({ error: "unauthorized" }), {
      status: 401,
      headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": env.SITE_URL ?? "*" },
    });
  }

  const testMode    = new URL(request.url).searchParams.get("test") === "true";
  const supabaseUrl = env.VITE_SUPABASE_URL;
  const serviceKey  = env.SUPABASE_SERVICE_ROLE_KEY;
  const resendKey   = env.RESEND_API_KEY;
  const siteUrl     = env.SITE_URL ?? "https://wingspot.netlify.app";
  const cors        = { "Content-Type": "application/json", "Access-Control-Allow-Origin": env.SITE_URL ?? "*" };
  ...
  await Promise.all(spotIds.map(async id => {
    ...
    try { forecastMap[id] = await getForecast(spot, supabaseUrl, serviceKey, forecastUrl, ensUrl, openmeteoKey); }
